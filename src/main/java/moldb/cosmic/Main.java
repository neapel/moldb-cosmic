...
public class Main {
	public static void main(String[] args) {		
		...
		if (entry != null) {
			...
			System.out.println("primary acc " + entry.getPrimaryUniProtAccession().getValue());

			for(SecondaryUniProtAccession a: entry.getSecondaryUniProtAccessions())
			...
			for(Comment x : entry.getComments(CommentType.ALTERNATIVE_PRODUCTS)) {
				for(AlternativeProductsIsoform k : ((AlternativeProductsComment)x).getIsoforms()) {
